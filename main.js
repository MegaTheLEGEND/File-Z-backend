...
function playAudio(source) {
  if (Array.isArray(source)) {
    // If 'source' is an array, select a random entry and play the associated URL.
    const randomIndex = Math.floor(Math.random() * source.length);
    const randomEntry = source[randomIndex];
    if (typeof randomEntry === 'object' && randomEntry.hasOwnProperty('name') && randomEntry.hasOwnProperty('url')) {
      const audio = new Audio(randomEntry.url);
      audio.play();
    } else {
      console.error('Invalid array entry:', randomEntry);
    }
  } else if (typeof source === 'string') {
    if (source.startsWith('http')) {
      // If 'source' is a string starting with 'http', treat it as a custom URL and play the audio.
      const audio = new Audio(source);
      audio.play();
    } else {
      // Otherwise, treat it as a predefined sound name and play that sound.
...
// Play audio from a custom URL:
playAudio('https://example.com/sound.mp3');

// Play a specific sound effect by name:
playAudio('sound1');

// Play a random sound effect from an array of objects with "name" and "url":
const soundEffectsArray = [
  { name: 'sound1', url: 'sound1.mp3' },
  { name: 'sound2', url: 'sound2.mp3' },
  { name: 'sound3', url: 'sound3.mp3' }
];
playAudio(soundEffectsArray);
